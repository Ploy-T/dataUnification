...
def upload_extract_vendor(fileName, masterFileName, blob):
    ...
    print(json_obj)
    page_1_vendor = json_obj["documentResults"][0]["fields"]["VendorName"]["text"]
    # text = extract_fr_json_text(page_1_lines)
    vendorName = {'verdorName': page_1_vendor}
    ...
    vendor = vendorMatch.match(vendorName,masterData)

    json_obj["refinedVendorName"] = vendor
    print(json_obj)
...
def main(req: func.HttpRequest) -> func.HttpResponse:
    ...
    if req_body:
        ...
        vendorMasterPath = req_body["vendorNamesMasterFilePath"]
        containerName= req_body['containerName']
        # upload_container_path = req_body['containerName']
        #fileName = file_path_json['fileName']
        accountName = req_body['accountName']
        try:
        
            azure_blob_file = AzureBlobFile(accountName,containerName,vendorMasterPath)
            blob = azure_blob_file.getBlobStream(fileName,masterFileName)

            output = upload_extract_vendor(fileName, masterFileName, blob)
